...
public class TorneoDaoMock implements ITorneoDao {
    ...
    public boolean authenticate(String alias, String password) {
        ...
        for (int i = 0; i < usuarios.length; i++) {
            if (usuarios[i] != null){
                if (alias.equals(usuarios[i].getAlias())&&password.equals(usuarios[i].getPassword())){
                    return true;
                }
            }
        }
    ...
    public Usuario getUsuarioByAlias(String alias) {
        ...
        for (int i = 0; i < usuarios.length; i++) {
            if (usuarios[i]!= null && alias.equals(usuarios[i].getAlias())){
                return usuarios[i];
            }
        }
        return null;
    ...
    private String getNombreEquipo(int idEquipo) {
        ...
        for (Usuario usuario : usuarios){
            if (usuario != null && usuario.getId() == idEquipo && usuario.isEsEquipo()){
    ...
    public Torneo getTorneo(int idTorneo) {
        ...
        for (Torneo torneo : torneos) {
            if (torneo != null && torneo.getId() == idTorneo) {
                return torneo;
            }
        }
        return null;
    ...
    public Torneo getTorneo(String nombreTorneo) {
        ...
        for (Torneo torneo : torneos) {
            if (torneo != null && torneo.getNombre().equals(nombreTorneo)){
                return torneo;
            }
        }
    ...
    public Fase getFase(int idTorneo) {
        ...
        for (Fase fase : fases) {
            if (fase != null && fase.getId() == idTorneo){
                return fase;
            }
        }
    ...
    private int getNumeroRegistrosFase(int idFase) {
        ...
        for (int i = 0; i < fases.length; i++) {
            if (fases[i] != null && idFase < fases.length){
                return fases[i].getNumeroEquipos() / 2;
            }
        }
    ...
    public RegistroResultadosDto[] getResultadosTorneo(int idTorneo) {
        ...
        int rddVariable = 0;
        for (Registro registro : DataDaoMock.registros) {
            if (registro.getTorneo() == idTorneo) {

                // Obtener nombres de equipos
                String nombreEq1 = DataDaoMock.usuarios[registro.getEquipo1() - 1].getNombre();
                String nombreEq2 = DataDaoMock.usuarios[registro.getEquipo2() - 1].getNombre();

                RegistroResultadosDto dto = new RegistroResultadosDto(nombreEq1, nombreEq2, new int[6], registro.getGanador());

                // Buscar los resultados de sets para este partido
                for (Resultado resultado : DataDaoMock.resultados) {
                    if (resultado.getRegistro() == registro.getId()) {
                        int numSet = resultado.getNumero();
                        int baseIndex = 2 * (numSet - 1);

                        // Llenar el array de 6 valores (valores[0], valores[1] para Set 1, etc.)
                        dto.getValores()[baseIndex] = resultado.getValor1();
                        dto.getValores()[baseIndex + 1] = resultado.getValor2();
                    }
                }

                rrd[rddVariable++] = dto;
                if (rddVariable >= rrd.length) break;
            }
        }
        return rrd;
    }
    ...
    public RegistroResultadosDto[] getResultadosTorneoFase(int idTorneo, int idFase) {
        ...
        // Itera sobre todos los registros de partidos
        for (Registro registro : DataDaoMock.registros) {
            // Filtra por Torneo Y por Fase
            if (registro != null && registro.getTorneo() == idTorneo && registro.getFase() == idFase) {

                if (rrdIndex >= n) break; // Límite de registros para la fase

                // Obtener nombres de equipos (se asume que los IDs son válidos)
                String nombreEq1 = DataDaoMock.usuarios[registro.getEquipo1() - 1].getNombre();
                String nombreEq2 = DataDaoMock.usuarios[registro.getEquipo2() - 1].getNombre();
                ...
                // Buscar los resultados de sets para este partido
                for (Resultado resultado : DataDaoMock.resultados) {
                    if (resultado != null && resultado.getRegistro() == registro.getId()) {
                        int numSet = resultado.getNumero();
                        int baseIndex = 2 * (numSet - 1);
    ...
    public RegistroResultadosDto[] getResultadosTorneoFases(int idTorneo, int idFase1, int idFase2) {
        ...
        System.arraycopy(rrd1, 0, rrd, 0, rrd1.length);

        // 2. Copiar los elementos de rrd2 justo después de los de rrd1
        System.arraycopy(rrd2, 0, rrd, rrd1.length, rrd2.length);
    ...
    public void revolverEquipos(int idTorneo) {
        ...
        int j = 0;
        for (int k = 0; k < DataDaoMock.registros.length; k++) {
            Registro registro = DataDaoMock.registros[k];

            if (registro != null && registro.getTorneo() == idTorneo && registro.getFase() == FASE_SIN_EMPEZAR_ID) {
                if (i < 4) { // Solo queremos los 4 primeros registros (Cuartos de final)
                    posRegistros[i++] = k;

                    equipoIds[j++] = registro.getEquipo1();
                    equipoIds[j++] = registro.getEquipo2();
                }
            }
        }

        // Si no encontramos los 4 registros, salir.
        if (i != 4) return;

        // 3. Revolver los equipos (32 intercambios aleatorios usando Math.random())
        for (int k = 0; k < 32; k++) {
            // Generar dos índices aleatorios entre 0 y 7
            int idx1 = (int) (Math.random() * 8);
            int idx2 = (int) (Math.random() * 8);

            // Intercambio (swap)
            int temp = equipoIds[idx1];
            equipoIds[idx1] = equipoIds[idx2];
            equipoIds[idx2] = temp;
        }

        // 4. Fijar los equipos revolvidos en los registros originales
        for (int k = 0; k < 4; k++) {
            int posRegistro = posRegistros[k];
            Registro registro = DataDaoMock.registros[posRegistro];

            // Asignar los IDs revolvidos a los equipos 1 y 2 del registro
            registro.setEquipo1(equipoIds[2 * k]);
            registro.setEquipo2(equipoIds[2 * k + 1]);
        }
    ...
    private boolean checkFinFase(int idTorneo, int idFaseActual) {
        ...
        if (idFaseActual == FASE_TERMINADO_ID) {
            return false;
        }

        // Flag para la comprobación combinada de las últimas fases
        boolean checkFasesFinales = (idFaseActual == FASE_TERCER_CUARTO_ID || idFaseActual == FASE_FINAL_ID);

        // Recorrer todos los registros
        for (Registro registro : DataDaoMock.registros) {
            if (registro != null && registro.getTorneo() == idTorneo) {

                boolean esFaseAComprobar = isEsFaseAComprobar(idFaseActual, registro, checkFasesFinales);

                // Si el registro pertenece a la fase/fases a comprobar y NO tiene ganador
                if (esFaseAComprobar) {
                    if (registro.getGanador() == 0) {
                        return false; // No se puede avanzar, hay partidos sin terminar
                    }
                }
            }
        }
    ...
    private static boolean isEsFaseAComprobar(int idFaseActual, Registro registro, boolean checkFasesFinales) {
        boolean esFaseAComprobar = false;

        if (checkFasesFinales) {
            // Si la fase actual es Tercer puesto o Final, comprobamos ambos partidos (Fase 3 y Fase 4)
            if (registro.getFase() == FASE_TERCER_CUARTO_ID || registro.getFase() == FASE_FINAL_ID) {
                esFaseAComprobar = true;
            }
        } else {
            // Para el resto de fases (Cuartos, Semifinales, Sin Empezar), comprobamos solo la fase actual
            if (registro.getFase() == idFaseActual) {
                esFaseAComprobar = true;
            }
        }
        return esFaseAComprobar;
    }
...
// 1. Poner la fase del torneo a Cuartos de Final (FASE_CUARTOS_ID)
        torneo.setFase(FASE_CUARTOS_ID);

        // 2. Recorrer todos los registros para actualizar la fase de los 4 partidos
        for (Registro registro : DataDaoMock.registros) {
            // Filtramos por el torneo y la fase actual (Sin Empezar)
            if (registro != null && registro.getTorneo() == torneo.getId() && registro.getFase() == FASE_SIN_EMPEZAR_ID) {

                // Poner la fase del registro a Cuartos de Final
                registro.setFase(FASE_CUARTOS_ID);

                // NOTA: Se asume que solo hay 4 registros en FASE_SIN_EMPEZAR_ID.
                // Podríamos añadir un contador para optimizar y salir al encontrar 4.
            }
        }
        return true;
    }
    ...
    private boolean avanzarFaseCuartos(Torneo torneo) {
        ...
        // 1. Comprobar si se puede avanzar
        if (!checkFinFase(idTorneo, FASE_CUARTOS_ID)) {
            return false;
        }
        ...
        // 2. Extraer los 4 IDs de los equipos ganadores de Cuartos
        for (Registro registro : DataDaoMock.registros) {
            if (registro.getTorneo() == idTorneo && registro.getFase() == FASE_CUARTOS_ID) {
                // Se asume que getGanador() es 1 o 2 (dado que checkFinFase es true)
                int ganadorEquipoId = (registro.getGanador() == 1) ? registro.getEquipo1() : registro.getEquipo2();
                ganadores[winnerIndex++] = ganadorEquipoId;
            }
        }

        // 3. Definir los partidos de Semifinales con los ganadores
        int semifinalMatchIndex = 0;
        for (Registro registro : DataDaoMock.registros) {
            if (registro.getTorneo() == idTorneo && registro.getFase() == FASE_SEMIFINALES_ID) {

                // Emparejamiento: (ganadores[0] vs. ganadores[1]) y (ganadores[2] vs ganadores[3])
                registro.setEquipo1(ganadores[semifinalMatchIndex * 2]);
                registro.setEquipo2(ganadores[semifinalMatchIndex * 2 + 1]);
                registro.setGanador(0); // Reiniciar

                semifinalMatchIndex++;
                if (semifinalMatchIndex >= 2) break; // Solo hay 2 partidos de semifinales
            }
        }

        // 4. Actualizar la fase del torneo
        torneo.setFase(FASE_SEMIFINALES_ID);
    ...
    private boolean avanzarFaseSemifinales(Torneo torneo) {
        ...
        // 1. Comprobar si se puede avanzar
        if (!checkFinFase(idTorneo, FASE_SEMIFINALES_ID)) {
            return false;
        }
        ...
        // 2. Extraer los IDs de ganadores (a Final) y perdedores (a Tercer Puesto)
        for (Registro registro : DataDaoMock.registros) {
            if (registro != null && registro.getTorneo() == idTorneo && registro.getFase() == FASE_SEMIFINALES_ID) {

                // Asignar ganador y perdedor según el valor de getGanador() (1 o 2)
                if (registro.getGanador() == 1) {
                    ganadores[index] = registro.getEquipo1();
                    perdedores[index] = registro.getEquipo2();
                } else { // getGanador() == 2
                    ganadores[index] = registro.getEquipo2();
                    perdedores[index] = registro.getEquipo1();
                }
                index++;
                if (index >= 2) break; // Solo hay 2 partidos de semifinales
            }
        }

        // 3. Definir los partidos de Tercer Puesto y Final
        for (Registro registro : DataDaoMock.registros) {
            if (registro != null && registro.getTorneo() == idTorneo) {

                if (registro.getFase() == FASE_TERCER_CUARTO_ID) {
                    // Partido por el Tercer Puesto (Perdedores vs. Perdedores)
                    registro.setEquipo1(perdedores[0]);
                    registro.setEquipo2(perdedores[1]);
                    registro.setGanador(0);

                } else if (registro.getFase() == FASE_FINAL_ID) {
                    // Partido de la Final (Ganadores vs. Ganadores)
                    registro.setEquipo1(ganadores[0]);
                    registro.setEquipo2(ganadores[1]);
                    registro.setGanador(0);
                }
            }
        }
    ...
    private boolean avanzarFaseFinales(Torneo torneo) {
        ...
        // 1. Comprobar si se puede avanzar de fase.
        // Como las fases FASE_TERCER_CUARTO_ID (3) y FASE_FINAL_ID (4) se juegan a la vez,
        // la comprobación (checkFinFase) debe verificar que ambos registros tengan ganador.
        if (!checkFinFase(idTorneo, FASE_FINAL_ID)) {
            return false;
        }

        // 2. Si ambos partidos han terminado, avanzar la fase del torneo a Terminado.
